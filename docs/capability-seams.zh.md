...
flowchart LR
  ...
  pkg_video_editor["video-editor"]
  svc_videoEditor["ctx.videoEditor<br/>Structured video-editing seam"]
  pkg_video_ffmpeg["video-ffmpeg"]
  pkg_spill["spill"]
  ...
  pkg_video_editor --> svc_videoEditor
  pkg_video_ffmpeg --> svc_videoEditor
  pkg_web --> svc_web
  ...
  svc_videoEditor --> pkg_tool_video
...
| `ctx.jobs` | `seam` | [`jobs`](../packages/jobs/jobs) | [`jobs-local`](../packages/jobs/jobs-local) | [`tool-bash`](../packages/shell/tool-bash), [`tool-terminal`](../packages/terminal/tool-terminal), [`tool-subagent`](../packages/subagent/tool-subagent), [`tool-video`](../packages/video/tool-video), [`tool-jobs`](../packages/jobs/tool-jobs) | - | 生产方（后台 bash、PTY 发送、subagent 委派和视频渲染）登记正在运行的工作；tool-jobs 是面向模型的控制器，用于读取、列出和终止这些工作；jobs-local 是进程本地注册表。 |
...
| `ctx.videoEditor` | `seam` | [`video-editor`](../packages/video/video-editor) | [`video-ffmpeg`](../packages/video/video-ffmpeg) | [`tool-video`](../packages/video/tool-video) | - | FFmpeg Provider 负责媒体分析和已验证渲染；tool-video 授权工作区路径，并向模型提供有界的分析与定时文本方案。 |
