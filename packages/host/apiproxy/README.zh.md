...
Workspace 上传是与之对应的宿主侧二进制写入面，同样不是 RPC：`POST /api/workspace.upload?sessionId=…&filename=…` 接收 `application/octet-stream`，解析已附加 Session 记录的 workspace，并用独占创建语义写入一个文件。文件名必须是一个路径片段，空请求体会被拒绝，已有目标返回 409 而不会被覆盖。`ApiProxy.uploads.workspaceFile` 收到完整字节之前，`/api` bridge 会先执行 `maxRequestBodyBytes` 限制。
